...
def register_routes(app):
    ...
    # Mapa: nombre_rol en DB → clave del select del formulario
    PERFIL_MAP = {
        "Ventas":        "ventas",
        "Logística":     "logistica",
        "Almacén":       "almacen",
    ...
    def login():
        ...
        if request.method == 'POST':
            ...
            # Traer usuarios y roles directamente de la API (sin endpoint nuevo)
            usuarios_data, err1 = api_get("/v1/usuarios/")
            roles_data,    err2 = api_get("/v1/roles/")

            if err1 or err2 or usuarios_data is None:
                error = "No se pudo conectar con la base de datos. Intenta de nuevo."
            else:
                usuarios = (
                    usuarios_data.get("usuarios", usuarios_data)
                    if isinstance(usuarios_data, dict)
                    else usuarios_data
                )
                rol_map = {r["id"]: r["nombre_rol"] for r in (roles_data or [])}
                usuario = next(
                    (u for u in usuarios if u.get("email", "").lower() == correo),
                    None
                )

                if not usuario:
                    error = "Correo no encontrado."
                elif not usuario.get("activo", True):
                    error = "Esta cuenta está dada de baja."
                elif usuario.get("password_hash", "") != password:
                    error = "Contraseña incorrecta."
                else:
                    nombre_rol = rol_map.get(usuario["rol_id"], "")
                    perfil_real = PERFIL_MAP.get(nombre_rol, "")
                    if perfil and perfil != perfil_real:
                        error = "El departamento seleccionado no coincide con tu cuenta."
                    else:
                        ...
                        if nombre_rol == "Ventas": return redirect(url_for('ventas_dashboard'))
                        if nombre_rol == "Logística": return redirect(url_for('logistica_dashboard'))
                        if nombre_rol == "Almacén": return redirect(url_for('almacen_dashboard'))
                        if nombre_rol == "Administrador": return redirect(url_for('admin_dashboard'))
    ...
    @app.route('/almacen/reportes', methods=['GET'])
    def almacen_reportes():
        redir = solo_almacen()
        if redir: return redir
        return render_template('almacen_reportes.html')
